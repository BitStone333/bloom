...
//! Helper metafunction to define a \c slist_hook that yields to the same
//! type when the same options (either explicitly or implicitly) are used.
#if defined(BOOST_INTRUSIVE_DOXYGEN_INVOKED) || defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
template<class ...Options>
#else
...
//! Put a public data member slist_hook in order to store objects of this class in
//! an list. slist_hook holds the data necessary for maintaining the list and
      ...
      >::type
{
   ...
   //!   user to do some additional work. \c swap can be used to emulate
   //!   move-semantics.
   slist_hook& operator=(const slist_hook& );
   ...
   //!
   //! <b>Throws</b>: Nothing.
   void swap_nodes(slist_hook &other);
