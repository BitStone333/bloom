...
//! Helper metafunction to define a \c bs_set_hook that yields to the same
//! type when the same options (either explicitly or implicitly) are used.
#if defined(BOOST_INTRUSIVE_DOXYGEN_INVOKED) || defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
template<class ...Options>
#else
...
//! Put a public data member bs_set_hook in order to store objects of this class in
//! a bs_set/bs_multiset. bs_set_hook holds the data necessary for maintaining the
      ...
      ::type
{
   ...
   //!   user to do some additional work. \c swap can be used to emulate
   //!   move-semantics.
   bs_set_hook& operator=(const bs_set_hook& );
   ...
   //!
   //! <b>Throws</b>: Nothing.
   void swap_nodes(bs_set_hook &other);
