...
//! Helper metafunction to define a \c unordered_set_hook that yields to the same
//! type when the same options (either explicitly or implicitly) are used.
#if defined(BOOST_INTRUSIVE_DOXYGEN_INVOKED) || defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
template<class ...Options>
#else
template<class O1 = void, class O2 = void, class O3 = void, class O4 = void>
#endif
struct make_unordered_set_hook
{
   /// @cond
   typedef typename pack_options
      < hook_defaults,
...
//! Put a public data member unordered_set_hook in order to store objects of this class in
//! an unordered_set/unordered_multi_set. unordered_set_hook holds the data necessary for maintaining the
...
class unordered_set_hook
   :  public make_unordered_set_hook<
         #if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
         O1, O2, O3, O4
         #else
         Options...
   ...
   >::type
{
   ...
   //!   user to do some additional work. \c swap can be used to emulate
   //!   move-semantics.
   unordered_set_hook& operator=(const unordered_set_hook& );
   ...
   //!
   //! <b>Throws</b>: Nothing.
   void swap_nodes(unordered_set_hook &other);
