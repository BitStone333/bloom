...
//! Helper metafunction to define a \c avl_set_hook that yields to the same
//! type when the same options (either explicitly or implicitly) are used.
#if defined(BOOST_INTRUSIVE_DOXYGEN_INVOKED) || defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
template<class ...Options>
#else
...
//! Put a public data member avl_set_hook in order to store objects of this class in
//! an avl_set/avl_multiset. avl_set_hook holds the data necessary for maintaining the
      ...
      ::type
{
   ...
   //!   user to do some additional work. \c swap can be used to emulate
   //!   move-semantics.
   avl_set_hook(const avl_set_hook& );
   ...
   //!   user to do some additional work. \c swap can be used to emulate
   //!   move-semantics.
   avl_set_hook& operator=(const avl_set_hook& );
   ...
   //!
   //! <b>Throws</b>: Nothing.
   void swap_nodes(avl_set_hook &other);
