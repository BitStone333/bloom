...
public:
    typedef void result_type;
    close_operation(T& t, BOOST_IOS::openmode which) 
        : t_(t), which_(which) 
        { }
    void operator()() const { t_.close(which_); }
private:
    close_operation& operator=(const close_operation&);
    T&                   t_;
    BOOST_IOS::openmode  which_;
};

template<typename T>
close_operation<T> call_close(T& t, BOOST_IOS::openmode which) 
{ return close_operation<T>(t, which); }
