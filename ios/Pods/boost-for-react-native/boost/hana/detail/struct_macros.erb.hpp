...
BOOST_HANA_NAMESPACE_BEGIN namespace struct_detail {
    ...
    constexpr auto prepare_name_impl(std::index_sequence<i...>) {
        return hana::string_c<hana::at_c<n>(Names::get())[i]...>;
    }

    template <std::size_t n, typename Names>
    constexpr auto prepare_name() {
        constexpr std::size_t len = strlen(hana::at_c<n>(Names::get()));
        return prepare_name_impl<n, Names>(std::make_index_sequence<len>{});
    }
    ...
    namespace boost { namespace hana {                                                            \
        ...
        struct accessors_impl<TYPE> {                                                             \
            static constexpr auto apply() {                                                       \
                ...
                return ::boost::hana::make_tuple(                                                 \
                    <%= (1..n).map { |i| "::boost::hana::make_pair(::boost::hana::struct_detail::prepare_name<#{i-1}, names>(), ::boost::hana::struct_detail::ptr<decltype(&TYPE::m#{i}), &TYPE::m#{i}>{})" }.join(', ') %>\
                );                                                                                \
            }                                                                                     \
        };                                                                                        \
    }}                                                                                            \
    ...
    namespace boost { namespace hana {                                                                  \
        ...
        struct accessors_impl<TYPE> {                                                                   \
            ...
            static constexpr auto apply() {                                                             \
                ...
                return ::boost::hana::make_tuple(                                                       \
                    <%= (1..n).map { |i| "::boost::hana::make_pair(::boost::hana::struct_detail::prepare_name<#{i-1}, names>(), BOOST_HANA_PP_DROP_FRONT m#{i})" }.join(', ') %>\
                );                                                                                      \
            }                                                                                           \
        };                                                                                              \
    }}                                                                                                  \
...
#define BOOST_HANA_DEFINE_STRUCT_IMPL_<%= n+1 %>(TYPE <%= (1..n).map { |i| ", m#{i}" }.join %>)       \
  ...
  struct hana_accessors_impl {                                                                        \
    static constexpr auto apply() {                                                                   \
      ...
      return ::boost::hana::make_tuple(                                                               \
        <%= (1..n).map { |i| "::boost::hana::make_pair(::boost::hana::struct_detail::prepare_name<#{i-1}, names>(), ::boost::hana::struct_detail::ptr<decltype(&TYPE::BOOST_HANA_PP_BACK m#{i}), &TYPE::BOOST_HANA_PP_BACK m#{i}>{})" }.join(', ') %>\
      );                                                                                              \
    }                                                                                                 \
  }                                                                                                   \
/**/
