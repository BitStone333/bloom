...
// sp_access, return type of operator->

template< class T > struct sp_access
{
    typedef T * type;
};
...
template< class T > struct sp_access< T[] >
{
    typedef void type;
};
...
template< class T, std::size_t N > struct sp_access< T[N] >
{
    typedef void type;
};
...
// Move support
    ...
    // never throws (but has a BOOST_ASSERT in it, so not marked with BOOST_NOEXCEPT)
    typename boost::detail::sp_access< T >::type operator-> () const 
    {
        BOOST_ASSERT( px != 0 );
        return px;
    }
...
#ifndef BOOST_NO_TEMPLATE_FRIENDS
