...
#   define BOOST_MPL_HAS_INTROSPECTION_NAME(args) \
      BOOST_PP_CAT(BOOST_PP_ARRAY_ELEM(0, args) , _introspect) \
    /**/

#   define BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_NAME(args, n) \
      BOOST_PP_CAT(BOOST_PP_CAT(BOOST_PP_ARRAY_ELEM(0, args) , _substitute), n) \
    /**/

#   define BOOST_MPL_HAS_INTROSPECTION_TEST_NAME(args) \
...
#   define BOOST_MPL_HAS_MULTI_SUBSTITUTE(z, n, args) \
      template< \
          template< BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), typename V) > class V \
       > \
      struct BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_NAME(args, n) { \
      }; \
    /**/

#   define BOOST_MPL_HAS_SUBSTITUTE(args, substitute_macro) \
      BOOST_PP_REPEAT( \
          BOOST_PP_ARRAY_ELEM(2, args) \
        , BOOST_MPL_HAS_MULTI_SUBSTITUTE \
        , args \
      ) \
    /**/

#   if !BOOST_MPL_HAS_XXX_NO_EXPLICIT_TEST_FUNCTION
#     define BOOST_MPL_HAS_REJECT(args, macro) \
        template< typename V > \
        static boost::mpl::aux::no_tag \
        BOOST_MPL_HAS_INTROSPECTION_TEST_NAME(args)(...); \
      /**/
#   else
#     define BOOST_MPL_HAS_REJECT(args, macro) \
        static boost::mpl::aux::no_tag \
...
#     define BOOST_MPL_HAS_MULTI_ACCEPT(z, n, args) \
        template< typename V > \
        static boost::mpl::aux::yes_tag \
        BOOST_MPL_HAS_INTROSPECTION_TEST_NAME(args)( \
            boost::mpl::aux::type_wrapper< V > const volatile* \
          , BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_NAME(args, n) < \
                V::template BOOST_PP_ARRAY_ELEM(1, args) \
            >* = 0 \
        ); \
      /**/
#     define BOOST_MPL_HAS_ACCEPT(args, macro) \
        BOOST_PP_REPEAT( \
            BOOST_PP_ARRAY_ELEM(2, args) \
          , BOOST_MPL_HAS_MULTI_ACCEPT \
          , args \
        ) \
      /**/
#   else
...
#   if !BOOST_MPL_HAS_XXX_NO_EXPLICIT_TEST_FUNCTION
#     define BOOST_MPL_HAS_TEST(args) \
          sizeof(BOOST_MPL_HAS_INTROSPECTION_TEST_NAME(args)< U >(0)) \
              == sizeof(boost::mpl::aux::yes_tag) \
      /**/
#   else
#     if !BOOST_MPL_HAS_XXX_NO_WRAPPED_TYPES
...
#   define BOOST_MPL_HAS_INTROSPECT( \
               args, substitute_macro, macro \
           ) \
      template< typename U > \
      struct BOOST_MPL_HAS_INTROSPECTION_NAME(args) { \
          BOOST_MPL_HAS_SUBSTITUTE(args, substitute_macro) \
          BOOST_MPL_HAS_REJECT(args, macro) \
          BOOST_MPL_HAS_ACCEPT(args, macro) \
          BOOST_STATIC_CONSTANT( \
              bool, value = BOOST_MPL_HAS_TEST(args) \
          ); \
          typedef boost::mpl::bool_< value > type; \
      }; \
    /**/

#   define BOOST_MPL_HAS_IMPLEMENTATION( \
               args, introspect_macro, substitute_macro, macro \
           ) \
      template< \
          typename T \
        , typename fallback_ \
      ...
      class BOOST_PP_ARRAY_ELEM(0, args) { \
          introspect_macro(args, substitute_macro, macro) \
      public: \
          static const bool value \
              = BOOST_MPL_HAS_INTROSPECTION_NAME(args)< T >::value; \
          typedef typename BOOST_MPL_HAS_INTROSPECTION_NAME(args)< \
              T \
          >::type type; \
      }; \
    /**/

// BOOST_MPL_HAS_WITH_FUNCTION_SFINAE expands to the full
// implementation of the function-based metafunction. Compile with -E
// to see the preprocessor output for this macro.
#   define BOOST_MPL_HAS_WITH_FUNCTION_SFINAE( \
               args, substitute_macro, macro \
           ) \
      BOOST_MPL_HAS_IMPLEMENTATION( \
          args \
        , BOOST_MPL_HAS_INTROSPECT \
        , substitute_macro \
        , macro \
      ) \
    /**/
...
#       define BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_NAME_WITH_TEMPLATE_SFINAE( \
                   args, n \
               ) \
          BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_NAME(args, n) \
        /**/
...
#     define BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_TAG_NAME( \
                 args \
             ) \
        BOOST_PP_CAT( \
            BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_NAME_WITH_TEMPLATE_SFINAE( \
                args, 0 \
            ) \
          , _tag \
        ) \
...
#     define BOOST_MPL_HAS_MULTI_SUBSTITUTE_WITH_TEMPLATE_SFINAE( \
                 z, n, args \
             ) \
        template< \
             template< BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), typename U) > class U \
        > \
        struct BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_NAME_WITH_TEMPLATE_SFINAE( \
                args, n \
               ) { \
            typedef \
                BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_TAG_NAME(args) \
                type; \
        }; \
      /**/

#     define BOOST_MPL_HAS_SUBSTITUTE_WITH_TEMPLATE_SFINAE( \
        ...
        BOOST_PP_REPEAT( \
            BOOST_PP_ARRAY_ELEM(2, args) \
          , BOOST_MPL_HAS_MULTI_SUBSTITUTE_WITH_TEMPLATE_SFINAE \
          , args \
        ) \
      /**/

#     define BOOST_MPL_HAS_REJECT_WITH_TEMPLATE_SFINAE( \
                 args, macro \
             ) \
        template< \
            typename U \
          , typename V \
                = BOOST_MPL_HAS_INTROSPECTION_SUBSTITUTE_TAG_NAME(args) \
        > \
        struct BOOST_MPL_HAS_INTROSPECTION_TEST_NAME(args) { \
            BOOST_STATIC_CONSTANT(bool, value = false); \
            typedef boost::mpl::bool_< value > type; \
        }; \
      /**/

#     define BOOST_MPL_HAS_MULTI_ACCEPT_WITH_TEMPLATE_SFINAE( \
                 z, n, args \
             ) \
        template< typename U > \
        struct BOOST_MPL_HAS_INTROSPECTION_TEST_NAME(args)< \
...
#     define BOOST_MPL_HAS_ACCEPT_WITH_TEMPLATE_SFINAE( \
                 args, macro \
             ) \
        BOOST_PP_REPEAT( \
            BOOST_PP_ARRAY_ELEM(2, args) \
          , BOOST_MPL_HAS_MULTI_ACCEPT_WITH_TEMPLATE_SFINAE \
          , args \
        ) \
      /**/

#     define BOOST_MPL_HAS_INTROSPECT_WITH_TEMPLATE_SFINAE( \
                 args, substitute_macro, macro \
             ) \
        BOOST_MPL_HAS_REJECT_WITH_TEMPLATE_SFINAE(args, macro) \
        BOOST_MPL_HAS_ACCEPT_WITH_TEMPLATE_SFINAE(args, macro) \
        template< typename U > \
        struct BOOST_MPL_HAS_INTROSPECTION_NAME(args) \
            : BOOST_MPL_HAS_INTROSPECTION_TEST_NAME(args)< U > { \
        }; \
      /**/
 
// BOOST_MPL_HAS_WITH_TEMPLATE_SFINAE expands to the full
...
// defined BOOST_MPL_HAS_SUBSTITUTE_WITH_TEMPLATE_SFINAE needs
// to be expanded at namespace level before
// BOOST_MPL_HAS_WITH_TEMPLATE_SFINAE can be used.
#     define BOOST_MPL_HAS_WITH_TEMPLATE_SFINAE( \
                 args, substitute_macro, macro \
             ) \
        BOOST_MPL_HAS_SUBSTITUTE_WITH_TEMPLATE_SFINAE( \
            args, substitute_macro \
        ) \
        BOOST_MPL_HAS_IMPLEMENTATION( \
            args \
          , BOOST_MPL_HAS_INTROSPECT_WITH_TEMPLATE_SFINAE \
          , substitute_macro \
          , macro \
        ) \
      /**/
...
#     define BOOST_MPL_HAS_XXX_TEMPLATE_NAMED_DEF(trait, name, default_) \
        BOOST_MPL_HAS_WITH_FUNCTION_SFINAE( \
            ( 4, ( trait, name, BOOST_MPL_LIMIT_METAFUNCTION_ARITY, default_ ) ) \
          , BOOST_MPL_HAS_TEMPLATE_SUBSTITUTE_PARAMETER \
          , BOOST_MPL_HAS_TEMPLATE_ACCESS \
        ) \
      /**/
#   else
#     define BOOST_MPL_HAS_XXX_TEMPLATE_NAMED_DEF(trait, name, default_) \
        BOOST_MPL_HAS_WITH_TEMPLATE_SFINAE( \
            ( 4, ( trait, name, BOOST_MPL_LIMIT_METAFUNCTION_ARITY, default_ ) ) \
          , BOOST_MPL_HAS_TEMPLATE_SUBSTITUTE_PARAMETER \
          , BOOST_MPL_HAS_TEMPLATE_ACCESS \
        ) \
