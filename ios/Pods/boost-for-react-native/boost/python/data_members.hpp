...
namespace detail
{
  ...
  {
      typedef typename default_getter_policy<D>::type policies;
...
{
    return detail::make_getter(d, policies, is_pointer<D>(), 0L);
}
...
inline object make_getter(D& x)
{
    ...
    return detail::make_getter(x, policy, is_pointer<D>(), 0L);
}
...
inline object make_getter(D const& d)
{
    detail::not_specified policy
        = detail::not_specified(); // Suppress a SunPro warning
    return detail::make_getter(d, policy, is_pointer<D>(), 0L);
}
#  endif
...
{
    return detail::make_setter(x, policies, is_pointer<D>(), 0);
}
...
{
    return detail::make_setter(x, default_call_policies(), is_pointer<D>(), 0);
}
