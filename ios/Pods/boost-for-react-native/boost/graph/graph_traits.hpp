...
namespace boost {
    ...
    namespace detail {
#define BOOST_GRAPH_OR_VOID(name) \
      BOOST_MPL_HAS_XXX_TRAIT_DEF(name) \
      template <typename T> struct BOOST_JOIN(get_, name) {typedef typename T::name type;}; \
      template <typename T> struct BOOST_JOIN(get_opt_, name): \
        boost::mpl::eval_if_c< \
          BOOST_JOIN(has_, name)<T>::value, \
          BOOST_JOIN(get_, name)<T>, \
          boost::mpl::identity<void> > \
        {};
      BOOST_GRAPH_OR_VOID(adjacency_iterator)
      BOOST_GRAPH_OR_VOID(out_edge_iterator)
      BOOST_GRAPH_OR_VOID(in_edge_iterator)
      BOOST_GRAPH_OR_VOID(vertex_iterator)
      BOOST_GRAPH_OR_VOID(edge_iterator)
      BOOST_GRAPH_OR_VOID(vertices_size_type)
      BOOST_GRAPH_OR_VOID(edges_size_type)
      BOOST_GRAPH_OR_VOID(degree_size_type)
    }

    template <typename G>
    struct graph_traits {
#define BOOST_GRAPH_PULL_OPT_MEMBER(name) \
        typedef typename detail::BOOST_JOIN(get_opt_, name)<G>::type name;
